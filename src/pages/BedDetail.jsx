          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                {current && (
                  <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest border ${isVacated ? 'bg-slate-50 text-slate-400 border-slate-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
              ...
              {current ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                    <div className="w-12 h-12 bg-indigo-600 text-white rounded-lg flex items-center justify-center text-xl font-black shadow-lg shadow-indigo-100">
                  ...
                  {!isVacated && (
                    <div className="pt-4 grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="py-2.5 bg-slate-50 text-slate-900 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all border border-slate-200 shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="py-2.5 bg-rose-50 text-rose-600 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-rose-100 transition-all border border-rose-100 shadow-sm"
                      >
                        Transfer
                      </button>
                    </div>
                ...
                <div className="text-center py-10">
                  <div className="w-16 h-16 bg-slate-50 rounded-xl flex items-center justify-center text-slate-200 mx-auto mb-4 border border-slate-100">
                    <User size={32} />
                  </div>
                  <p className="text-slate-500 text-xs font-medium mb-6 px-4">No resident currently assigned to this bed.</p>
                  <button
                    onClick={() => setQuickAssignOpen(true)}
                    className="w-full py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
                  >
                    Quick Assign
                  </button>
                </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              ...
              <div className="space-y-2">
                {sortedHistory.length === 0 ? (
                  <div className="text-center py-6 border-2 border-dashed border-slate-100 rounded-xl text-slate-400 text-[10px] font-black uppercase tracking-widest">
                    No history found
                  </div>
                ) : (
                  sortedHistory.map((h, i) => (
                    <div key={i} className="group p-3 rounded-xl border border-slate-50 hover:border-indigo-100 hover:bg-indigo-50/30 transition-all cursor-pointer" onClick={() => { setHistoryItem(h); setHistoryOpen(true); }}>
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">Ledger</h3>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Financial Records & Collection</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={exportPaymentsCSV} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-50 text-slate-600 text-[9px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all border border-slate-200 shadow-sm">
                    <Download size={12} /> CSV
                  </button>
                  <button onClick={printPayments} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-50 text-slate-600 text-[9px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all border border-slate-200 shadow-sm">
                ...
                {!current || visiblePeriods.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {visiblePeriods.map(p => {
                      ...
                      return (
                        <div key={p.key} className="group p-4 rounded-xl border border-slate-100 hover:border-indigo-100 hover:shadow-lg hover:shadow-indigo-500/5 transition-all">
                                    ...
                                    {paid ? (
                                      <>
                                        <button
                                          ...
                                          className="p-2.5 bg-slate-50 text-slate-400 hover:text-indigo-600 rounded-xl transition-all border border-slate-200 shadow-sm"
                                          title="Print Receipt"
                                        >
                                          <Printer size={16} />
                                        </button>
                                      ...
                                      <button
                                        ...
                                        className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95"
                                      >
                                        Mark as Paid
                                      </button>
                                    )}
