        ...
        <div className="bg-slate-900/40 backdrop-blur-2xl rounded-2xl sm:rounded-3xl border border-slate-800/50 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] p-5 sm:p-7 md:p-8 relative overflow-hidden group">
          ...
          <motion.div variants={itemVariants} className="text-center mb-5 sm:mb-6">
            <div className="inline-flex relative mb-3">
              ...
              <div className="h-14 w-14 sm:h-16 sm:w-16 rounded-full bg-white p-0 relative z-10 shadow-xl flex items-center justify-center overflow-hidden">
                <img src={LogoImg} alt="ManageMyPg" className="w-full h-full object-contain" />
              </div>
            </div>
            <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight mb-0.5">Create Account</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <motion.div variants={itemVariants} className="space-y-1">
                ...
                <div className="relative group">
                  <User className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-indigo-400 transition-colors" size={15} />
                  <input
                    value={username}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <motion.div variants={itemVariants} className="space-y-1">
                ...
                <div className="relative group">
                  <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-indigo-400 transition-colors" size={15} />
                  <input
                    type="tel"
                    value={phone}
              ...
              <motion.div variants={itemVariants} className="space-y-1">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full bg-slate-950/40 border border-slate-800 rounded-xl pl-9 pr-9 py-2.5 text-xs sm:text-sm text-white placeholder:text-slate-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 outline-none transition-all"
                    required
                  />
                  <button
                    type="button"
            ...
            {/* Password Feedback */}
            <div className="flex flex-col gap-0.5 min-h-[14px]">
              {passwordError && <span className="text-[10px] font-bold text-rose-400 uppercase tracking-tight ml-1">{passwordError}</span>}
              {passwordMatch && <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-tight ml-1 flex items-center gap-1"><CheckCircle2 size={12}/> Passwords Match</span>}
            </div>

            {/* Checkbox */}
            <motion.div variants={itemVariants} className="flex items-center gap-2.5 ml-1 pt-0.5">
              <input
                id="agree-checkbox"
                type="checkbox"
                checked={agree}
                onChange={(e) => setAgree(e.target.checked)}
                className="w-3.5 h-3.5 rounded border-slate-800 bg-slate-950/40 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-slate-900 cursor-pointer shrink-0"
              />
              <label htmlFor="agree-checkbox" className="text-[9.5px] font-bold text-slate-400 uppercase tracking-wide leading-tight cursor-pointer">
                I agree to the <Link to="/terms-and-conditions" className="text-indigo-400 hover:underline">Terms</Link> & <Link to="/privacy-policy" className="text-indigo-400 hover:underline">Privacy Policy</Link>
              </label>
            </motion.div>
